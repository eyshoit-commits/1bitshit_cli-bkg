...
async fn main() -> Result<()> {
    ...
    if let Ok(current_exe) = std::env::current_exe() {
        let global_bin_dir = bitshit_shared::HardwareGovernor::resolve_bin_gateway();
    ...
    let log_path = bitshit_shared::environment::EnvironmentManager::current()
        .local_dir
    ...
    if let Ok(control) = bitshit_shared::hardware::governor::HardwareGovernor::load_system_control() {
    ...
    match cli.command {
        ...
        Some(CliCommand::Status) => {
            engines::telemetry::health_check::bitshitHealthChecker::run_full_benchmark();
        }
        ...
        Some(CliCommand::DevSync { target, driver_name, profile }) => {
            let global_dir = bitshit_shared::environment::EnvironmentManager::current().global_dir;
            println!("⚙️  [DevSync] Manually synchronizing '{}' development artifacts to {}...", target, global_dir.display());
            if let Err(_e) = bitshit_shared::HardwareGovernor::resolve_engine_path().parent().unwrap().symlink_metadata() {
                let _ = std::fs::create_dir_all(bitshit_shared::HardwareGovernor::resolve_engine_path());
            }
            ...
            let _ = bitshit_shared::hardware::governor::HardwareGovernor::load_system_control();
            ...
            if let Ok(pkg_data) = std::fs::read_to_string("package.json") {
                if let Ok(json) = serde_json::from_str::<serde_json::Value>(&pkg_data) {
                    let _ = bitshit_shared::hardware::governor::RegistryGovernor::seal_registry(json);
                }
            }
