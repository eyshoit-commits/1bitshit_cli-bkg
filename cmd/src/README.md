# 🧠 bitshit Source Architecture (`cmd/src`)

Welcome to the central nervous system of the bitshit ecosystem. This directory contains the sovereign source code for the Command Line Interface (CLI) and the immersive Ratatui-based Terminal User Interface (TUI). 
...
- **Sovereign Ghost Execution Guard:** Ensures the binary is running from the master `~/.bitshit/bin` path.
- **CLI Argument Parser:** Uses `clap` to handle one-off commands (e.g. `bitshit dev-sync`, `bitshit brain on`).
...
* **`bootstrapper.rs`**: The heartbeat of bitshit. 
  * Checks for missing components.
  * Handles artifact synchronization (e.g., `dev-sync`).
  * Fetches `package.json` to verify engine versions.
  * *Law of the Engine:* bitshit cannot launch until the bootstrapper successfully seals the registry.
...
## ⚙️ How Booting Works (The bitshit Lifecycle)

When you type `bitshit` in your terminal:

1. **Ignition (`main.rs`)**: 
   The CLI checks if it's running from the correct global path. It then parses args.
2. **Bootstrapper Validation**:
   `Bootstrapper::ignite()` verifies that the Neural Foundry (permissions) and Skills exist. It ensures `~/.bitshit/engine` is populated.
...
3. **No Direct Hardware Calls here:** The CLI should always use `bitshit-shared` or the `engines` API crate to talk to the GPU/Drivers. The UI must remain decoupled from the hardware execution logic.
