...
use bitshit_shared::hardware::schema::booster::{
...
pub async fn execute(
    ...
    if has_args {
        ...
        println!("\n  {} {}", "🚀".cyan(), "bitshit Booster - Interactive Performance Setup".bold());
            ...
            match choice {
                ...
                "DFlash" => {
                    ...
                    if let Ok(d) = inquire::Select::new("DFlash (FlashKDA):", dflash_opts).with_help_message("").prompt() {
                        control.dflash = bitshit_shared::hardware::schema::booster::SmartState::Static(d.to_string());
                        let _ = HardwareGovernor::save_booster_settings(&control);
                    }
                }
                "Response Length" => {
