use bitshit_shared::backend::traits::{bitshitInference, UnifiedBackend};
use bitshit_shared::hardware::governor::HardwareGovernor;
use bitshit_shared::hardware::schema::booster::FeatureState;
use bitshit_shared::{bitshitContext, StructuralDNA, TemplateManager};
...
pub async fn execute(target_model_id: Option<String>, runs: usize) -> Result<()> {
    ...
    for model in full_roster {
        let path_str = if let Some(local_path) = &model.local_path {
            ...
            let models_dir = bitshit_shared::environment::EnvironmentManager::current()
                .models_dir()
                .join("chat");
...
fn get_benchmark_out_dir() -> PathBuf {
    let mut base = if cfg!(debug_assertions) {
        ...
        while let Some(name) = path.file_name() {
            if name.to_string_lossy() == "bitshit" {
                break;
            }
            if !path.pop() {
                break;
        ...
        bitshit_shared::environment::EnvironmentManager::current()
            .local_dir
            .join("reports")
            .join("benchmark")
    };
...
async fn run_single_model_isolated(model_name: &str, runs: usize) {
    ...
    let context = bitshitContext::boot(dna.clone(), TemplateManager::default());
    ...
    for prompt_def in suite.prompts {
        ...
        let mut prompt_report_md = format!(
            "# 🚀 bitshit Hardware Benchmark Report\n\n\
            ## 🤖 Model: {}\n\
