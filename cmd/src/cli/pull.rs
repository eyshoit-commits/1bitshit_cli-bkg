...
pub async fn execute(model_id: &str) -> Result<()> {
    ...
    let bitshit_root = bitshit_shared::environment::EnvironmentManager::current()
        .ensure_models_dir()
        .unwrap_or_else(|_| bitshit_shared::environment::EnvironmentManager::current().models_dir());
    ...
    // Pre-load the engine into the state
    {
        ...
        lock.active_backend = engines::api::router::Backend::bitshit(engine);
    }
