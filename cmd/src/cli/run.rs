...
/// `bitshit run <model-id>` — pulls the model and initiates a native chat session.
pub async fn execute(model_id: &str, _interactive: bool) -> Result<()> {
    ...
    println!("\n  {} [bitshit] Initializing Kernel for '{}'...", "⚙️".yellow(), model_id.bold());
    ...
    let bitshit_root = bitshit_shared::environment::EnvironmentManager::current().models_dir();
    ...
    if model_id.contains('/') {
        ...
        if let Some(existing) = roster.iter().find(|m| m.huggingface_repo.to_lowercase() == repo_id.to_lowercase() && m.huggingface_filename.to_lowercase() == selected_filename.to_lowercase()) {
            ...
            println!("     To run it instantly, use: bitshit run {}", existing.id.green());
            println!("     If you wish to re-download, please delete the old one first using: bitshit rm {}\n", existing.id.red());
            return Ok(());
        }
        ...
        if let Some(m) = roster.into_iter().find(|m| m.id.to_lowercase() == model_id.to_lowercase()) {
            ...
            let model_path = bitshit_root.join(&m.category).join(&safe_id);
    ...
    let skills_dir = bitshit_shared::environment::EnvironmentManager::current().skills_dir();
    ...
    if _interactive {
        ...
        loop {
            ...
            let res = tokio::task::block_in_place(|| -> Result<(), color_eyre::eyre::Report> {
                ...
                let pipe_name = r"\\.\pipe\bitshit_engine_pipe";
                let mut client = match std::fs::OpenOptions::new()
                    ...
                    Ok(client) => client,
                    Err(e) => {
                        return Err(color_eyre::eyre::eyre!("❌ Failed to connect to Native Daemon IPC (Is bitshit daemon running?): {}", e));
                    }
                };
                            ...
                            while let Some(pos) = accum_line.find('\n') {
                                ...
                                if let Ok(val) = serde_json::from_str::<serde_json::Value>(&line) {
                                    ...
                                    if token.starts_with("__STEP_2_MATCH_START__") {
                                        ...
                                        let matched = if parts.len() >= 2 { parts[1] } else { "bitshit-search" };
