...
pub async fn execute() -> Result<()> {
    ...
    let model_path = bitshit_shared::environment::EnvironmentManager::current().models_dir().join("chat").join("bonsai1-8b").join("Bonsai-8B.gguf");
    ...
    let dna = bitshit_shared::StructuralDNA::default();
    let context = bitshit_shared::bitshitContext::boot(dna, bitshit_shared::TemplateManager::default());
    ...
    router.active_backend = engines::api::router::Backend::bitshit(engine);

    // Boot router indices
    if let Ok(mut g_router) = bitshit_shared::skills::router::GLOBAL_SKILL_ROUTER.write() {
    ...
    if res.is_ok() {
        ...
        let cache_file = bitshit_shared::environment::EnvironmentManager::current().skills_dir().join("minimax-music-gen").join(".cache").join("bonsai1-8b.kvcache.bin");
