...
pub async fn run_native(
    ...
    if !state.printed_logo {
        ...
        println!();
        println!("  {} {}", "1BitShit".cyan().bold(), "v0.2.0".bright_black());
    ...
    loop {
        ...
        let ans = match Select::new("bitshit Main Menu:", options)
        ...
        match ans {
            ...
            "🌐 Server Control" => {
                ...
                if let Ok(s_ans) = Select::new("Server Control:", s_opts).with_render_config(config.clone()).prompt() {
                    ...
                    match s_ans {
                        "🚀 Start API Daemon" => {
                            println!("  {} Starting bitshit API Daemon on http://localhost:8000 ...", "🚀".green());
