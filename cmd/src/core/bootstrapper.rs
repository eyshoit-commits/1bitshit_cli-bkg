...
use bitshit_shared::HardwareGovernor;
...
impl Bootstrapper {
    const MASTER_REGISTRY_URL: &'static str = "https://raw.githubusercontent.com/bitshit/bitshit/main/package.json";

    /// 🚀 bitshit BOOTSTRAP: The Sovereign Handshake.
    pub async fn ignite(is_dev_sync: bool) -> Result<()> {
        let local_dir = bitshit_shared::environment::EnvironmentManager::current().local_dir;
        ...
        tracing::info!("🧠 [bitshit] Igniting Neural Foundry (Permissions & Skills)...");
        ...
        let env = bitshit_shared::environment::EnvironmentManager::current();
        ...
        let hub_path = bitshit_shared::HardwareGovernor::resolve_hub_path();
        ...
        let client = reqwest::Client::builder()
            .user_agent(format!("bitshit-Bootstrapper/{}", env!("CARGO_PKG_VERSION")))
            .build()?;

        // 🎯 1. Fetch Master Registry (package.json)
        tracing::info!("🛰️ [bitshit] Synchronizing Neural Registry...");
        ...
        let master_registry: serde_json::Value = match master_registry_res {
            Ok(res) if res.status().is_success() => {
                match res.json().await {
                    Ok(json) => json,
                    Err(_) => {
                        println!("  {} [bitshit] Network sync skipped (invalid registry format).", "⚠️".yellow());
                        return Ok(());
                    }
                }
            },
            _ => {
                println!("  {} [bitshit] Network sync skipped (offline mode).", "⚠️".yellow());
                return Ok(());
            }
        };
        
        // 🏛️ Seal the Master Registry with Atomic Write Protocol
        bitshit_shared::RegistryGovernor::seal_registry(master_registry.clone())
        ...
        if latest_cli != current_cli && !latest_cli.is_empty() {
            println!("  {} [1BitShit] Update Available: {} -> {}", "🚀".green(), current_cli, latest_cli);
        }
        ...
        let engine_marker = engine_dir.join("bitshit-engine.ready");
        ...
        if !engine_path.exists() || local_version != manifest_version {
            println!("  {} [bitshit] Provisioning Core Engine ({})...", "⚙️".yellow(), manifest_version);
            ...
            match async {
                ...
                Err(e) if engine_path.exists() => {
                    println!("  {} [bitshit] Provisioning failed ({}). Using cached engine.", "⚠️".yellow(), e);
                }
                Err(e) => {
    ...
    async fn sync_neural_stack(master_registry: &serde_json::Value) -> Result<()> {
        ...
        let kernel_path = kernel_dir.join(format!("bitshit-llama.{}", kernel_ext));
        let kernel_marker = kernel_dir.join("bitshit-llama.ready");
        ...
        if !kernel_path.exists() || local_version != manifest_version {
            println!("  {} [bitshit] Synchronizing Neural Kernel ({})...", "📦".magenta(), manifest_version);
            let client = reqwest::Client::builder().user_agent(format!("bitshit-Bootstrapper/{}", env!("CARGO_PKG_VERSION"))).build()?;
            ...
            match async {
                ...
                Err(e) if kernel_path.exists() => {
                    println!("  {} [bitshit] Kernel sync failed ({}). Using cached kernel.", "⚠️".yellow(), e);
                }
                Err(e) => {
    ...
    /// 🛠️ Artifact Sync: Synchronizes local build artifacts to .bitshit.
    ...
    pub fn sync_dev_artifacts(target: &str, driver_name: Option<&str>, hub_path: PathBuf, profile: &str) -> Result<()> {
        ...
        // 1. Engine Sync (engines.dll -> bitshit-engine.dll)
        if target == "all" || target == "core" {
            let engine_dest = hub_path.join("engine").join(format!("bitshit-engine.{}", ext));
            ...
            if let Some((engine_src, is_release)) = find_artifact("engines", ext) {
                ...
                if let Err(e) = copy_with_rename(&engine_src, &engine_dest) {
                    ...
                    let _ = std::fs::write(hub_path.join("engine").join("bitshit-engine.ready"), marker);
        ...
        let kernels_to_sync = vec![
            ("bitshit_llama", "bitshit-llama"),
            ("bitshit_onnx", "bitshit-onnx"),
        ...
        // 3. CLI Executable Sync (bitshit.exe -> bin/bitshit.exe)
        if target == "all" || target == "core" {
            let exe_name = if cfg!(windows) { "bitshit.exe" } else { "bitshit" };
            let target_dir = if profile == "release" { &release_dir } else { &debug_dir };
        ...
        if target == "all" || target == "core" {
            let local_bitshit = root.join(".bitshit");
            
            // Sync Brain
            let local_brain = local_bitshit.join("brain");
            ...
            // Sync Skills
            let local_skills = local_bitshit.join("skills");
    ...
    /// 🌐 Automatically injects bitshit into the Windows Global PATH.
    /// Just like Ollama or Bun, users won't need to manually configure environment variables.
    fn ensure_global_path() {
        #[cfg(windows)]
        {
            let bin_dir = bitshit_shared::HardwareGovernor::resolve_bin_gateway();
            let bin_str = bin_dir.to_string_lossy().to_string();
            
            // Simple PowerShell script to read User PATH, check if bitshit is in it, and append if missing.
            let script = format!(
