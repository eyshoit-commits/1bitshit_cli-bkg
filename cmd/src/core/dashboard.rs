...
use tokio::sync::mpsc;
use dispatcher::EmbeddingDriver;
// ── 📦 MODULAR APPS ──
...
impl DashboardEngine {
    pub fn run_native(
        ...
        // ══ 🔒 bitshit RENDER CONFIG ══
        let config = RenderConfig::default();

        // 🛑 GRACEFUL INTERRUPT HANDLER (Sovereign Pivot Control)
        let _ = ctrlc::set_handler(move || {
            bitshit_shared::GLOBAL_CANCEL_SIGNAL.store(true, Ordering::SeqCst);
        });

        // ── 🧬 ATOMIC Core DISCOVERY (bitshit Startup Scan) ──
        ...
        // ── 📡 bitshit TELEMETRY IGNITION (Ghost Observer Singleton) ──
        let state_pulse = bitshit_shared::hardware::telemetry::get_pulse();
        ...
        // 🚀 bitshit AUTO-BOOT: Activate the latest engine silently only if no model is loaded
            ...
            if !perms.lazy_load_model {
                match boot_target {
                    Some((ref name, Some(ref path_str))) => {
                        ...
                        let mut spinner = bitshit_shared::utils::spinner::bitshitSpinner::new();
                        ...
                        let runtime = if is_gguf {
                            bitshit_shared::BackendType::RuntimeB
                        } else {
                            bitshit_shared::BackendType::RuntimeA
                        };
        ...
        let mut last_booster_modified = std::fs::metadata(bitshit_shared::environment::EnvironmentManager::current().engine_dir().join("system_booster.json")).and_then(|m| m.modified()).ok();
        let mut last_booster_state = bitshit_shared::hardware::governor::HardwareGovernor::load_booster_settings().ok();
            ...
            match input {
                Ok(val) => {
                    ...
                    if final_message.starts_with('/') {
                        ...
                        let booster_path = bitshit_shared::environment::EnvironmentManager::current().engine_dir().join("system_booster.json");
                        if let Ok(meta) = std::fs::metadata(&booster_path) {
                            if let Ok(modified) = meta.modified() {
                                let mut needs_reload = false;
                                if let Some(last) = last_booster_modified {
                                    if modified > last {
                                        if let Ok(current_booster) = bitshit_shared::hardware::governor::HardwareGovernor::load_booster_settings() {
                        ...
                        bitshit_shared::GLOBAL_CANCEL_SIGNAL.store(false, Ordering::SeqCst);
                        ...
                        let stream_result = tokio::task::block_in_place(|| {
                            ...
                            let booster = bitshit_shared::hardware::governor::HardwareGovernor::load_booster_settings().unwrap_or_default();
                            let suppress_thinking = booster.think_mode == bitshit_shared::hardware::schema::booster::FeatureState::Off;
                            ...
                            let result = lock.generate_stream(
                                ...
                                Box::new(move |token: String| -> bool {
                                    ...
                                    if (eos_cb.load(Ordering::SeqCst) && !token.starts_with("__ENGINE_PAUSE_EXECUTE__")) || bitshit_shared::GLOBAL_CANCEL_SIGNAL.load(Ordering::SeqCst) { 
                                        return false; 
                                    }
                                        ...
                                        if let Ok(crossterm::event::Event::Key(key)) = crossterm::event::read() {
                                            if key.code == crossterm::event::KeyCode::Char('t') && key.modifiers.contains(crossterm::event::KeyModifiers::CONTROL) {
                                                ...
                                                if in_think_cb.load(Ordering::SeqCst) {
                                                    bitshit_shared::GLOBAL_SKIP_THINKING_SIGNAL.store(true, Ordering::SeqCst);
                                    ...
                                    if let Ok(mut res) = full_clone.lock() {
                                        ...
                                        if token.starts_with("__STEP_2_MATCH_START__") {
                                            ...
                                            let mut sp = bitshit_shared::utils::spinner::bitshitSpinner::new();
                        ...
                        if bitshit_shared::GLOBAL_CANCEL_SIGNAL.load(Ordering::SeqCst) {
                            println!();
                            use crossterm::style::Stylize;
                        ...
                        let registry = bitshit_shared::hardware::governor::HardwareGovernor::load_process_registry();
        ...
        let (master_ans, ans) = if let Some(target) = auto_boot_target {
            ...
            loop {
                ...
                if master_ans.contains("Engine Modes") {
                    ...
                    let mut booster = bitshit_shared::hardware::governor::HardwareGovernor::load_booster_settings().unwrap_or_default();
                    if mode_ans.contains("Flash Mode") {
                        booster.mode_run = bitshit_shared::hardware::schema::booster::BoosterMode::Edge;
                        booster.think_mode = bitshit_shared::hardware::schema::booster::FeatureState::Off;
                    } else if mode_ans.contains("Think Mode") {
                        booster.mode_run = bitshit_shared::hardware::schema::booster::BoosterMode::MaxBoost;
                        booster.think_mode = bitshit_shared::hardware::schema::booster::FeatureState::On;
                    } else if mode_ans.contains("Boot Mode") {
                        booster.mode_run = bitshit_shared::hardware::schema::booster::BoosterMode::Balance;
                        booster.think_mode = bitshit_shared::hardware::schema::booster::FeatureState::Auto;
                    }
                    
                    let _ = bitshit_shared::hardware::governor::HardwareGovernor::save_booster_settings(&booster);
                    ...
                    loop {
                        ...
                        if key_part.as_str() == "Compute Device" {
                            ...
                            if let Ok(_) = bitshit_shared::hardware::governor::HardwareGovernor::save_booster_settings(&booster) {
                        ...
                        if key_part.as_str() == "Neural Mode" {
                            ...
                            booster.mode_run = match selected_mode.as_str() {
                                "edge" => bitshit_shared::hardware::schema::booster::BoosterMode::Edge,
                                "multitasking" => bitshit_shared::hardware::schema::booster::BoosterMode::Multitasking,
                                "balance" => bitshit_shared::hardware::schema::booster::BoosterMode::Balance,
                                "max_boost" => bitshit_shared::hardware::schema::booster::BoosterMode::MaxBoost,
                                "ultra_max_boost" => bitshit_shared::hardware::schema::booster::BoosterMode::UltraMaxBoost,
                                "hyper_cluster" => bitshit_shared::hardware::schema::booster::BoosterMode::HyperCluster,
                                _ => bitshit_shared::hardware::schema::booster::BoosterMode::Balance,
                            };
                        ...
                        if key_part.as_str() == "Context Shifting" {
                            ...
                            booster.context_shifting = match selected_shift.as_str() {
                                "Off" => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Off,
                                "Minimal" => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Minimal,
                                "Standard" => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Standard,
                                "Aggressive" => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Aggressive,
                                "Extreme" => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Extreme,
                                _ => bitshit_shared::hardware::schema::booster::ContextShiftingMode::Auto,
                            };
                        ...
                        if key_part.as_str() == "KV Cache Quantization" {
                            ...
                            booster.kv_cache_quantization = match selected_kv.as_str() {
                                s if s.starts_with("16-bit") => bitshit_shared::hardware::schema::booster::KvCacheQuantization::Kv16,
                                s if s.starts_with("8-bit") => bitshit_shared::hardware::schema::booster::KvCacheQuantization::Kv8,
                                s if s.starts_with("4-bit") => bitshit_shared::hardware::schema::booster::KvCacheQuantization::Kv4,
                                _ => bitshit_shared::hardware::schema::booster::KvCacheQuantization::Auto,
                            };
                        ...
                        let feature_state = match val_ans.as_str() {
                            "On" => bitshit_shared::hardware::schema::booster::FeatureState::On,
                            "Off" => bitshit_shared::hardware::schema::booster::FeatureState::Off,
                            _ => bitshit_shared::hardware::schema::booster::FeatureState::Auto,
                        };
                        ...
                        match key_part.as_str() {
                            ...
                            "DFlash" => {
                                booster.dflash = match val_ans.as_str() {
                                    "On" => bitshit_shared::hardware::schema::booster::SmartState::Static("On".to_string()),
                                    "Off" => bitshit_shared::hardware::schema::booster::SmartState::Static("Off".to_string()),
                                    _ => bitshit_shared::hardware::schema::booster::SmartState::Static("Auto".to_string()),
                                };
                            },
                    ...
                    let result = tokio::task::block_in_place(|| {
                        ...
                        match handle.block_on(engines::CoreRouter::load_model(
                            ...
                            Err(e) => {
                                ...
                                if runtime == bitshit_shared::BackendType::RuntimeB
