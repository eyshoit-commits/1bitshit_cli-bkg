# 🧿 bitshit Neural Hub: bitshit-DNA CLI (Architecture & Master Source of Truth)

This document serves as the **Single Source of Truth** and the **Architectural Brain** for the bitshit CLI (The "Neural OS" Dashboard). It defines the deep logic, user experience sequences, CLI toolchain, and hardware-level governance that power the bitshit ecosystem.

---

## 💻 1. The Command Arsenal (Complete CLI Reference)

The bitshit CLI provides deep, bare-metal control over local AI orchestration.

### 🧩 Neural Skills & Sandboxed Agents
```bash
$ bitshit skill install <skill_name>   # Install a WASM Sandboxed Skill
$ bitshit skill list                   # List active OS-level skills
$ bitshit skill cache ls               # View generated SSD KV Caches
$ bitshit skill cache clear <model>    # Clear a specific corrupted cache
$ bitshit skill cache clear --all      # Nuke all orphaned caches globally
```

### 🧠 Core Engine & Evaluation
```bash
$ bitshit benchmark                    # Process-isolated full system VRAM stress test
$ bitshit benchmark bonsai1-8b --runs 3 # Iterative thermal-throttled evaluation
$ bitshit run <model_id>               # Launch CLI stream via C++ FFI Kernel
$ bitshit test-jit                     # Dynamic Pipeline Diagnostic & Memory Check
```

### 🗄️ Vault Management & Identity
```bash
$ bitshit pull <model_id>              # Fetch weights silently (Pre-flight Hardware Audit)
$ bitshit rm <model_id>                # Permanently free up SSD physical blocks
$ bitshit list                         # Active CoreRoster visualization
$ bitshit ps                           # Auto-heal stale PIDs and monitor memory
$ bitshit ingest <file_path>           # RAG: Semantic chunking via ONNX gatekeepers
$ bitshit setup profile                # LMDB Vectorization of Node Identity
```

### 🎛️ Bare-Metal Control
```bash
$ bitshit --calibrate                  # Real-time RDTSC hardware clock & SIMD profiling
$ bitshit booster                      # Interactive Hardware Tuning (Quant, FlashKDA)
$ bitshit brain <on/off/status>        # Manage bitshitdb FFI Daemon gateway
$ bitshit                              # Launch the Full TUI Interactive Dashboard
```
...
Every top-tier neural engine must establish a "Neural Handshake" with the user. bitshit's onboarding (`src/ui/apps/onboarding/`) is a sequential, animated journey that defines the **Structural DNA** of the local node.

### **Phase A: Neural Ignition (The Intro)**
- **Logic**: A 60-frame mathematical ASCII animation sequence (`ritual.rs`).
- **Implementation**: The `bitshit` logo starts in `Color::DarkGray` and pulses into `Color::Cyan` over 1 second, providing a true active "boot-up" feel.

### **Phase B: bitshit Identity & Native Extraction**
bitshit must know who it is serving to optimize the neural weights:
...
bitshit adopts extreme performance hacks and deep architectural patterns to ensure maximum bare-metal optimization:
...
bitshit's state is divided into robust asynchronous dimensions:
...
---
**bitshit Neural Hub: Built for the bitshit-DNA Node. 🧿⚡🚀**
